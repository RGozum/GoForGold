...
const { Attendance, sequelize, Student_Enrollment } = require('../models');
...
router.get("/attendancedata/:year_id", async(req,res) => {
...
for (i=0; i<dates.length; i++) {
    let sqlString = "SELECT se.activities_id, se.student_id, a.attendance_date, u.first_name, u.last_name, se.year_id " +
	"FROM users u JOIN student_enrollment se ON u.user_id = se.student_id " +
	  ...
	  "WHERE se.activities_id = " + activity_id + " AND se.year_id = " + year_id + " ";
...
// router.get("/attendancedata", isAuthenticated, hasRole(ADMIN, FACULTY), async(req,res) => {
//   const activity_id = req.query.activity_id;
//   console.log("Activity:" +activity_id);
...
router.post("/update-attendance", isAuthenticated, hasRole(ADMIN, FACULTY), async (req,res)=> {
  const {student_id, activity_id, date,value, year_id}=req.body;
